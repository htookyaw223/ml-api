...
module.exports.getEfficiencyForecast = ( siteId,startDate, endDate, equipId, key) => {
  ...
  const query1 =` SELECT ts, UNIX_TIMESTAMP(ts)*1000 as time, sum(${key}) ${key}, sum(${key}_lower) ${key}_lower, sum(${key}_upper) ${key}_upper, sum(${key}_pred) ${key}_pred
  FROM aimgmt.forecast where ts between '${startDate}' and '${endDate}' group by ts order by ts asc `
  
  const q = this.con.promise().query(equipId=='HD1'?query1:query)
  this.con.releaseConnection(this.con) // release connection
  return q
}
...
module.exports.getDeviceAndParameterMapping = (siteId) => {
  const query = `SELECT distinct dm.deviceId as equipId, 
...
module.exports.getDeviceListForEfficiency = (siteId) => {
  const query=`select dm.ieee,dm.dataType,dm.tableName,dm.columnName,d.id as deviceId, 
...
module.exports.getDeviceListForDeviceStatus = (siteId) => {
  ...
  on dm.deviceId=d.id and dm.dataType='kw' 
  where d.siteId =? and d.deviceType='CH'`
  // console.log(query)
  const q = this.con.promise().query(query, [siteId])
  this.con.releaseConnection(this.con) // release connection

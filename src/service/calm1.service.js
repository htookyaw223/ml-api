...
module.exports.getChillerHealthNewOld = async ({ siteId = "cl_tao", clientId }) => {
    console.log("getChillerHealthNew: ", clientId, siteId)
    const periodDays = 7
    const query = `
    select 
...
module.exports.calculateChillerHealth = async ({ siteId = "cl_tao", clientId }) => {
    try {
        let monthNo = new Date("2020-01-01").getMonth();
...
module.exports.chillerHealthMeter = async ({ clientId, siteId = "cl_tao", periodDays = 30, eDate = null }) => {
    ...
    // HK_TEST
    // const retdata = [{ "totalCount": 14007, "inefficientCount": "11725", "equipId": "CH1", "equipName": "Chiller1", "deviceId": "1", "inefficientDefText": "E > 0.7", "inefficientDefId": 15, "inefficientPercent": "83.7081", "healthEfficiency": 1, "efficiencyText": "Chiller1 operated inefficiently about 83% in the last 30 days.Requires operational improvement to maintain chiller health and save Energy.", "count": 1, "highDays": "0", "mediumDays": "0", "lowDays": "0", "healthSeverity": 3, "severityText": "", "siteId": "cl_pb", "hours": "0.0000", "minutes": "0", "faultyPercent": 0, "healthFaultyHour": 3, "faultyHourText": "", "rh": 13963, "increasedRate": -100, "healthIncreasedRate": 3, "faultyMinutes": 0, "rateText": "", "chillerHealth": 3, "chillerHealthRaw": 2.5 }, { "totalCount": 14007, "inefficientCount": "1848", "equipId": "CH2", "equipName": "Chiller2", "deviceId": "2", "inefficientDefText": "E > 0.7", "inefficientDefId": 16, "inefficientPercent": "13.1934", "healthEfficiency": 2, "efficiencyText": "Chiller2 operated inefficiently about 13% in the last 30 days.", "count": 24, "highDays": "0", "mediumDays": "8", "lowDays": "0", "healthSeverity": 2, "severityText": "Chiller2 had Severity Medium Issues repeatedly for 8 days in the last 30 days.", "siteId": "cl_pb", "hours": "15.7833", "minutes": "947", "faultyPercent": 20.740254051686378, "healthFaultyHour": 2, "faultyHourText": "Chiller2 faulty hour is 20% in the last 30 days.", "rh": 4566, "increasedRate": 286.60714285714283, "healthIncreasedRate": 1, "faultyMinutes": 0, "rateText": "", "chillerHealth": 2, "chillerHealthRaw": 1.75 }, {
    //     "totalCount": 43202, "inefficientCount": "6393", "equipId": "CH3", "equipName": "Chiller3", "deviceId": "3", "inefficientDefText": "E > 0.7", "inefficientDefId": 17, "inefficientPercent": "14.7979", "healthEfficiency": 2, "efficiencyText": "Chiller3 operated inefficiently about 14% in the last 30 days.", "count": 60, "highDays": "0", "mediumDays": "20", "lowDays": "0", "healthSeverity": 1, "severityText": "Chiller3 had Severity Medium Issues repeatedly for 20 days in the last 30 days.", "siteId": "cl_pb", "hours": "98.7000", "minutes": "5922", "faultyPercent": 42.61044754640956, "healthFaultyHour": 1,
    //     "faultyHourText": "Chiller3 faulty hour is42% in the last 30 days.", "rh": 13898, "increasedRate": 154.84992101105846, "healthIncreasedRate": 1, "faultyMinutes": 0, "rateText": "", "chillerHealth": 1, "chillerHealthRaw": 1.25
    // }, { "totalCount": 43202, "inefficientCount": "212", "equipId": "CH4", "equipName": "Chiller4", "deviceId": "4", "inefficientDefText": "E > 0.7", "inefficientDefId": 18, "inefficientPercent": "0.4907", "healthEfficiency": 3, "efficiencyText": "", "count": 1, "highDays": "0", "mediumDays": "0", "lowDays": "0", "healthSeverity": 3, "severityText": "", "siteId": "cl_pb", "hours": "0.0000", "minutes": "0", "faultyPercent": 0, "healthFaultyHour": 3, "faultyHourText": "", "rh": 8478, "increasedRate": -100, "healthIncreasedRate": 3, "faultyMinutes": 0, "rateText": "", "chillerHealth": 3, "chillerHealthRaw": 3 }]

    // return retdata;

    //console.log("chillerHealthMeter: start: ", Date.now() / 1000)

    // console.log("this.getListForCH({ siteId }): ", this.getListForCH({ siteId }))
    ...
    //const deviceStatusParameterList = (await db.getDeviceListForDeviceStatus(siteId))[0]
    // #Hk_test
    ...
    const deviceStatusCountQueryResults = deviceStatusCount.map(d => {
        const equip = deviceStatusParameterList.find(v => v.ieee == d.ieee)
        return ({ ...d, equipId: equip.equipId })
    })

    // console.log("deviceStatusCountQueryResults",deviceStatusCountQueryResults)

    // console.log("deviceStatusParameterList: ", deviceStatusParameterList)
    // const deviceStatusCountQuery = deviceStatusParameterList.map(v => {
    //     if (v.ieee === null) return `select 0 as deviceOnCount, 0 as deviceOffCount,'${v.equipId}' as equipId;`
    //     else return `SELECT sum(deviceOnCount) as deviceOnCount,sum(deviceOffCount) as deviceOffCount,'${v.equipId}' as equipId
    //     FROM (SELECT SUM(IF(device_status>0,1,0)) as deviceOnCount,SUM(IF(device_status=0,1,0)) as deviceOffCount
    //         FROM iotmgmt.deviceStatus 
    //         where ieee='cltao007b' and ts between '${startDate}' and '${endDate}' 
    //         group by date_format(ts,"%Y-%m-%d %h:%i:00")
    //     )t1;`
    // }).join("")
    // let deviceStatusCountQueryResults = (await mgmtdb.selectQuery(deviceStatusCountQuery))[0]
    // console.log("deviceStatusCountQueryResults: ", deviceStatusCountQueryResults)
    // New Changes: 9-Oct-2020 > No 1 & No 2
    /*
    1. Calculate running hours. If not possible, Count the total number of rows available for the parameter.
    2. Count Efficiency rows available. Count Efficiency Rows out of Prescribed Range. Calculate percentage of inefficient operation.
        For Eg. Chiller 1 – 43200 Rows for 1 month.
            No. Of rows that contain value > 0.8 = 12000 Rows
            Percentage = 12000/43200 * 100 = 27.7 %
            Chiller 1 operated inefficiently about 27 % of the time in the last 1 month.
    */
    // const efficiencyParameterList = (await db.getDeviceListForEfficiency(siteId))[0]
    // console.log("efficiencyParameterList: ", efficiencyParameterList)
    // const efficiencyCountsQuery = efficiencyParameterList.map(v => {
    //     if (v.ieee === null) return `SELECT 1 as totalCount, 0 as inefficientCount, 0 as inefficientPercent, '${v.equipId}' as equipId, '${v.deviceTypeName}' as equipName, '${v.deviceId}' as deviceId;
    //     `
    //     return `SELECT t.*, (inefficientCount/totalCount)*100 as inefficientPercent from (
    //     SELECT COUNT(*) as totalCount, SUM(IF(${v.columnName}>0.7, 1, 0)) as inefficientCount, '${v.equipId}' as equipId, '${v.deviceTypeName}' as equipName, '${v.deviceId}' as deviceId
    //     FROM iotmgmt.${v.tableName} 
    //     WHERE ieee='${v.ieee}' and ts between '${startDate}' and '${endDate}') t;
    //     `
    // }).join("")
    // let efficiencyCountsQueryResults = (await mgmtdb.selectQuery(efficiencyCountsQuery))[0]
    ...
    // console.log("efficiencyCountsQueryResults: ", efficiencyCountsQueryResults) // LOG result
    ...
    // from (
    //     SELECT c.severity_id, d.equipId, d.deviceTypeName, count(*) as count, count(distinct "'", date_format(dm.start_time, '%Y-%m-%d'), "'") as days
    //     FROM ibpem_db.device d
    //     LEFT JOIN iotdata.detection_master dm on dm.equip_id=d.equipId and algo_id='calm_v1' and dm.start_time>='${startDate}' and dm.end_time<='${endDate}'
    //     LEFT JOIN iotmgmt.dtd_calm c on c.detect_id=dm.detect_id
    //     WHERE d.deviceType='CH'
    //     GROUP BY d.equipId, c.severity_id
    // ) t
    // GROUP BY t.equipId`
    // // console.log("severityOccurenceQuery: ", severityOccurenceQuery)
    // let severityOccurenceQueryResults = (await mgmtdb.selectQuery(severityOccurenceQuery))[0]
    const severityOccurenceQueryResults = severityOccurenceQueryResults1.map(v => {
        ...
        //     d = v.highDays
        // }
        // else if (v.mediumDays > 10) {
        //     health = 1
        //     s = "Medium"
        //     d = v.mediumDays
        // }
        // else if (v.lowDays > 15) {
        //     health = 1
        //     s = "Low"
        //     d = v.lowDays
        // } // Poor
        // else if (v.highDays > 1) {
        //     health = 2
        //     s = "High"
        //     d = v.highDays
        // }
        // else if (v.mediumDays > 5) {
        //     health = 2
        //     s = "Medium"
        //     d = v.mediumDays
        // }
        ...
        // if(s===null) {
        //     d = Math.max(v.lowDays, v.mediumDays, v.highDays)
        //     s = ""
        // }
    ...
    // console.log("severityOccurenceQueryResults: ", severityOccurenceQueryResults) // LOG result

    // const faultyOperationHourResults = (await this.calculateTrendGraphDataCalmForDates({ siteId, startDate, endDate }))[0]
    //console.log(deviceStatusCountQueryResults,"deviceSttaus")
    const faultyOperationHourResults = faultyOperationHourResults1.map(v => {
        ...
        // const health = faultyPercent > 40 ? 1 : faultyPercent > 10 ? 2 : 3
        // const faultyHourText = faultyPercent===0 ? "" : `${v.equipName} faulty hour ${faultyPercent >= 0 ? 'increased' : 'decreased'} ${Math.abs(Math.floor(faultyPercent))}% in the last 30 days.`
    ...
    // console.log("faultyOperationHourResults: ", faultyOperationHourResults) // LOG result
    ...
    const finalResults = preResults.map(v => {
        // console.log(v)
        // const healths = [v.healthEfficiency, v.healthSeverity, v.healthFaultyHour, v.healthIncreasedRate].reduce((r, c) => {
        //     const R = { ...r }
        //     if (R[c]) R[c] += 1
        //     else R[c] = 1
        //     return R
        // }, {})
        // const chillerHealth = Object.keys(healths).reduce((r, c) => {
        //     if (r === null) return ({ key: c, value: healths[c] })
        //     return healths[c] > r.value ? ({ key: c, value: healths[c] }) : r
        // }, null)
        // return ({ ...v, chillerHealth: parseInt(chillerHealth.key) })
    ...
    // console.log("chillerHealthMeter: end: ", Date.now() / 1000)
    return finalResults
}
...
module.exports.getEnergySavings = async ({ siteId = "cl_tao", period = "year", toInsert = false, dateForMonth = null }) => { // period=year|month
    try {
        // const periodDays = 30
        const currentDate = dateForMonth ? new Date(dateForMonth) : kumoDate.Date()
        const currentYear = dateFns.format(currentDate, "yyyy")
        ...
        /*const queryMonthly = `select
            t.months,
            t.totalKW as totalKW,
            t.totalCoolingLoadCalc as inefficientKWH,
            round(t.totalCoolingLoadCalc / t.totalKW * 100) as inefficientPercent,
            t.faultyMinutes
        from (
            select month(rt.ts) as months, 
                sum(kw)/60 as totalKW, 
                sum(if(rt.equip_id like 'CH%', (rt.cooling_load * ef.eff / 60) - (rt.cooling_load * ${siteInefficientDef.value} / 60)  , 0)) as totalCoolingLoadCalc, sum(if(chw_flowrate<=0, 1, 0)) faultyMinutes
            ...
            and rt.site_id='${siteId}' and rt.kw>0 and ef.eff>${siteInefficientDef.value}
            group by month(rt.ts)
        )t
        group by t.months;`
        const queryYearly = `select
            t.totalKW as totalKW,
            t.totalCoolingLoadCalc as inefficientKWH,
            round(t.totalCoolingLoadCalc / t.totalKW * 100) as inefficientPercent,
            t.faultyMinutes
...
// this.calculateTrendGraphDataCalm({ siteId: "cl_tao", year: 2020 })
